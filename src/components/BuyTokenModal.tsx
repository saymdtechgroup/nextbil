...
export const BuyTokenModal: React.FC<BuyTokenModalProps> = ({
  ...
  const handleAddNxbcToWallet = async () => {
    try {
      ...
      if (!eth || typeof eth.request !== 'function') {
        navigator.clipboard.writeText('0x85363386808d1f26BF3805Bb44a093a2Af9E8783');
      ...
      await eth.request({
        ...
        params: {
          type: 'ERC20',
          options: {
            address: '0x85363386808d1f26BF3805Bb44a093a2Af9E8783',
            symbol: 'NXBC',
            decimals: 18,
          },
        },
  ...
  const executeWeb3Payment = async (): Promise<string | null> => {
    ...
    if (currency === 'USDT') {
      ...
      // Keccak256 hash for buyTokens(uint256) is 0x3610724e
      setPaymentStatusText(`Calling Smart Contract...`);
      const buyTokensData = `0x3610724e${cleanVal}`;
